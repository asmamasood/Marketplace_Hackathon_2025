...
const Form = () => {
    ...
    const [isDropOf, setIsDropOf] = useState(false);
    ...
    const CustomSubmit = async (d:any) => {
        const response = await fetch("https://api.web3forms.com/submit", {
            ...
            body: JSON.stringify({
                access_key: "a82751ac-ba14-471f-b93b-0f32b418ecec",
                name: d.name,
                number: d.number,
                address : d.address,
                city : d.city,
  ...
  return (
    <form onSubmit={handleSubmit(CustomSubmit)} className='order-2 lg:order-1'>
                    ...
                    <div className={`md:flex justify-between gap-3 mt-2  ${isPickUp ? "opacity-100" : "opacity-50 pointer-events-none"}`}>
                        <div className='md:w-[50%]'>
                            ...
                            <select disabled={!isPickUp} className='w-full p-3 bg-[#F6F7F9] rounded-lg mt-1'>
                                <option typeof='location' value="">Select your Location</option>
        ...
        <div>
                    <div className='flex gap-2'>
                        <input type="radio" checked={isDropOf}
                        onChange={() => setIsDropOf(!isDropOf)} />
                        <h1 className='font-[PlusJakartaSan] font-semibold text-[16px]'>Drop - Of</h1>
                    </div>

                    <div className={`md:flex justify-between gap-3 mt-2  ${isDropOf ? "opacity-100" : "opacity-50 pointer-events-none"}`}>
                        <div className='md:w-[50%]'>
                            <label className='text-[rgba(26,32,44,100)] font-bold text-[16px]' >Location</label>
                            <select disabled={!isDropOf} className='w-full p-3 bg-[#F6F7F9] rounded-lg mt-1'>
                                <option typeof='location' value="">Select your Location</option>
                                {locations.map((l:any, index:number)=>(
                                    <option key={index} value={l}>{l}</option>
                                ))}
                        ...
                        <div className='mt-2 md:mt-0  md:w-[50%]'>
                            <h1 className='text-[rgba(26,32,44,100)] font-bold text-[16px]'>Date</h1>
                            <input {...register("date")} disabled={!isDropOf} type="date" className='w-full p-3 bg-[#F6F7F9] rounded-lg mt-1' />
                        </div>
                    </div>
                    ...
                    <div className='mt-2'>
                        <div className='mt-2 md:mt-0 md:w-[50%]'>
                            <h1 className='text-[rgba(26,32,44,100)] font-bold text-[16px]'>Time</h1>
                            <input {...register("time")} disabled={!isDropOf} type="time" className='w-full p-3 bg-[#F6F7F9] rounded-lg mt-1' />
                        </div>
                    </div>
                </div>
            ...
            <div className='my-3 bg-white p-2'>
                    <label className='text-[rgba(26,32,44,100)] font-bold text-[16px]'>Is Driver Needed?</label>
                    <div className='flex gap-2'>
                        <input type="radio" value="yes" {...register("driverNeeded")} onChange={() => setIsDriverNeeded(true)} /> Yes
                        <input type="radio" value="no" {...register("driverNeeded")} onChange={() => setIsDriverNeeded(false)} /> No
                    </div>
                    {isDriverNeeded && <h1 className='text-[rgba(26,32,44,100)] font-bold'>Driver Rent: $10</h1>}
                </div>
